            ...
            conn.query(`select * from ${process.env.TABELA1} where email =  ?`, [req.body.email], (error,response)=>{
                ...
                if(response.length > 0){
                    ...
                    bcrypt.hash(req.body.password, 10, (errBcrypt, hash_password)=>{
                        ...
                        conn.query(`insert into ${process.env.TABELA1} (id_user, name,email,password, profile_photo, username, establishment, phone, type_user, address, user_cpf, establishment_cnpj, user_rg, token) values (?, ?,?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                        [id, req.body.name, email,hash_password,req.body.profile_photo, req.body.username, req.body.establishment, phone, req.body.type_user, address, user_cpf, establishment_cnpj, user_rg, token],
                        (error) => {
                        conn.release()
                        if(error){return res.status(500).json({error:error.sqlMessage})}
